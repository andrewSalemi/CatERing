...
public class Menu {
  ...
  public static Menu loadMenuById(int mid) {
    if (loadedMenus.containsKey(mid)) return loadedMenus.get(mid);

    Menu load = new Menu();
    final int[] menuOwner = new int[1];


    String menuQuery = "SELECT * FROM Menus WHERE id= '" + mid + "'";
    PersistenceManager.executeQuery(menuQuery, rs -> {
      load.id = rs.getInt("id");
      load.title = rs.getString("title");
      load.published = rs.getBoolean("published");
      menuOwner[0] = rs.getInt("owner_id");
    });
    load.owner = User.loadUserById(menuOwner[0]);

    String featuresQuery = "SELECT * FROM MenuFeatures WHERE menu_id = " + load.id;
    ...
    String inuseQuery = "SELECT * FROM Services WHERE approved_menu_id = " + load.id;
